...
class HatchbackProductionLine extends CarProductionLine{
  ...
  // out of Product Line
  setModel(){
    this.sedanCar.model = this.getInternalModel();// sedan <-  SedanProductionLine
  }
...
class Director{
  ...
  constructSportEdition(){
    this.productionLine.setAirBags(2).setColor('Electric Blue').setEdition('Sport');
  }
...
function appBuilder(director){
  ...
  const hatchbackProductionLine = new HatchbackProductionLine({ model: 'Rhino 2.0'});
  // order to create the product
  director.setProductionLine(hatchbackProductionLine);
  director.constructSignatureEdition();
  const rhinoHatchbackSignature = hatchbackProductionLine.build();

  console.log(rhinoHatchbackSignature);


  const hatchbackSportProductionLine = new HatchbackProductionLine({ model: 'C7'});
  // order to create the product
  director.setProductionLine(hatchbackSportProductionLine);
  director.constructSportEdition();
  const rhinoHatchbackSport = hatchbackSportProductionLine.build();
